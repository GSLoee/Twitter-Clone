...
def profile():
    ...
    if not g.user:
        flash(f'You do not have access to this')
        return redirect('/')
    ...
    if form.validate_on_submit():
        g.user.usename = form.username.data
...
@app.route('/users/un-like/<int:message_id>', methods=['POST'])
...
def homepage():
    ...
    if g.user:
        ...
        random_users = (db.session.query(User).filter(User.id.notin_(followed_user_ids))
